...
class ModelConfig:
    ...
    Attributes:
        ...
        dropout: FFN 내부 / 임베딩 / residual 덧셈 전에 공통으로 적용하는
            dropout 확률.
        attention_dropout: 어텐션 확률 맵에 적용하는 dropout (보통 dropout과
            다르게 두므로 따로 유지한다).
        activation: FFN 활성화 함수 이름 ("relu", "gelu", "silu", ...).
        bias: 선형 레이어(FFN / Q·K·V / 출력 head)가 bias 항을 쓰는지 여부.
    ...
    # vocab_size 3형제와 pad_token_id는 train.py가 어휘집으로부터 런타임에
    # 채운다 (YAML에 두면 오해를 부르므로 설정 파일에는 노출하지 않는다).
    vocab_size: int = 8000
...
class OptimizationConfig:
    ...
    Attributes:
        ...
        scheduler: "noam", "cosine", "linear" 또는 "constant".
            decay 없이 warm-up 후 고정하고 싶으면 "constant"를 쓴다.
...
class DatasetConfig:
    ...
    Attributes:
        ...
        tgt_lang: 활성 쌍의 타겟 언어 코드. preprocess.py는 기본으로 이 쌍만
            전처리하며, 여러 쌍을 만들려면 --lang-pairs로 넘긴다.
...
class InferenceConfig:
    ...
    Attributes:
        beam_size: 빔 서치에 사용할 빔 개수 (1이면 greedy로 대체됨).
...
class Config:
    ...
    @classmethod
    def from_dict(cls, raw: dict[str, Any], strict: bool = True) -> "Config":
        """일반 중첩 dict(예: 체크포인트에서 온)로부터 Config를 만든다.

        Args:
            raw: 섹션별로 중첩된 설정 dict.
            strict: True면 모르는 키/섹션에서 즉시 실패한다 (사람이 쓴 YAML의
                오타를 잡기 위함). False면 조용히 무시한다 — **예전 버전에서
                저장된 체크포인트**를 읽을 때 쓴다. 그 안의 설정은 기계가 쓴
                것이라 오타일 리 없고, 그 사이 제거된 하이퍼파라미터가 들어
                있을 수 있다. 무시된 키는 이 프로젝트에서 더 이상 아무 의미가
                없으므로 기본값을 쓰는 것이 곧 올바른 동작이다.
        """
        ...
        for name, section_cls in _SECTION_TYPES.items():
            ...
            if unknown:
                if strict:  # 오타를 조용히 무시하지 않고 바로 실패시킨다
                    raise ValueError(
    ...
    def validate(self) -> None:
        ...
        m, o, c = self.model, self.optimization, self.checkpoint
